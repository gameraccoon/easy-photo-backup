...
pub fn send_files(
    ...
    for i in 0..files.len() {
        ...
        std::mem::swap(&mut file_path, &mut files[i].path);
        let result = send_file(&file_path, files[i].root_path.as_ref(), stream);
        ...
        sent_files_cache.append(&file_path, &files[i].new_change_detection_data);
