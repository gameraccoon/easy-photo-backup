...
namespace TcpServer
{
	...
	static void handleClient(const Network::RawSocket socket, sockaddr /*clientAddr*/, socklen_t /*clientAddrLen*/, const std::filesystem::path& targetDir, ServerConfigStorage& configStorage, const std::function<void(Requests::PendingClientBinding&& pendingClientBinding)>& onPairingRequestFn)
	{
		...
		std::visit(
			VisitLambda{
				...
				[socket, &configStorage, &targetDir](const Requests::SendFiles&& sendFiles) {
					Requests::processSendFilesInteractiveRequest(sendFiles.connectionId, sendFiles.firstMessage, socket, configStorage, targetDir);
				},
			},
			std::move(request)
		);
	...
	std::optional<std::string> runServer(ServerConfigStorage& configStorage, const char* interfaceAddressStr, const Network::AddressType addressType, const std::filesystem::path& targetDir, std::promise<uint16_t>& portPromise, const std::function<void(Requests::PendingClientBinding&& pendingClientBinding)>& onPairingRequestFn)
	{
		...
		while (true)
		{
			...
			std::thread([connectionSocket, clientAddr, clientAddrLen, targetDir, &configStorage, &onPairingRequestFn] {
				Network::AutoclosingSocket socketGuard(connectionSocket);
				handleClient(connectionSocket, clientAddr, clientAddrLen, targetDir, configStorage, onPairingRequestFn);
			}).detach();
		}
