...
fn main() {
    ...
    let mut storage = ClientStorage::load_or_generate(&PathBuf::from(client_cli_processor::CLIENT_STORAGE_FILE_NAME));

    if storage.client_name.is_empty() {
        storage.client_name = config.client_name.clone();
        let result = storage.save(&PathBuf::from(client_cli_processor::CLIENT_STORAGE_FILE_NAME));
        if let Err(e) = result {
